document.addEventListener("DOMContentLoaded", function () {
  ...
  function sendEmailToTeacher(to_email, to_name, grade, password) {
    ...
    const params = { to_email, to_name, grade, password };
  ...
  function loadSubjectAllocations() {
    ...
    allocations.forEach((a, i) => {
      ...
      row.innerHTML = `
        ...
        <td class="action-col"><button onclick="deleteSubjectAlloc(${i})">🗑️</button></td>
      `;
  ...
  function loadClassAllocations() {
    ...
    allocations.forEach((a, i) => {
      ...
      row.innerHTML = `
        <td>${teacher ? teacher.firstname + " " + teacher.lastname : "Unknown"}</td>
        <td>Grade ${a.grade}</td>
        <td class="action-col"><button onclick="deleteClassAlloc(${i})">🗑️</button></td>
      `;
...
const adminName = loggedInUser.firstname || "Unknown";
const generatedBy = `Generated by: ${adminName} — ${new Date().toLocaleDateString()}`;

function generateHeader(doc, title) {
  const schoolName = "CBC Student Portal – Admin Report";
...
function exportTableToPDF(tableId, title, color, fileName) {
  const { jsPDF } = window.jspdf;
  const doc = new jsPDF("p", "mm", "a4");

  generateHeader(doc, title);

  // Temporarily hide action columns before export
  ...
  generateFooter(doc, generatedBy);
  doc.save(fileName);
}

document.getElementById("exportSubjectsBtn").addEventListener("click", () => {
  exportTableToPDF("#subjectAllocTable", "Teacher Subject Allocations", [46, 134, 193], "Subject_Allocations_Report.pdf");
});

document.getElementById("exportClassBtn").addEventListener("click", () => {
  exportTableToPDF("#classAllocTable", "Class Teacher Allocations", [39, 174, 96], "Class_Teacher_Allocations_Report.pdf");
});

// ===============================
// PROTECT ADMIN ROUTE + REFRESH
// ===============================
document.addEventListener("DOMContentLoaded", function () {
